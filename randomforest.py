...
def main():
  ...
  changeRF = RandomForestClassifier(
      n_estimators=300,
      max_depth=None,
      random_state=42,
      n_jobs=-1
  )
  changeRF.fit(X_train, Y_change_train)
  changePredictions = changeRF.predict(X_test)
  changeRFAccuracy = accuracy_score(Y_change_test, changePredictions)
  changeProba = changeRF.predict_proba(X_test)[:, 1]


  diabetesMedRF = RandomForestClassifier(
      n_estimators=300,
      max_depth=None,
      random_state=42,
      n_jobs=-1
  )
  diabetesMedRF.fit(X_train, Y_diabetesMed_train)
  diabetesMedPredictions = diabetesMedRF.predict(X_test)
  diabetesMedRFAccuracy = accuracy_score(Y_diabetesMed_test, diabetesMedPredictions)
  diabetesMedProba = diabetesMedRF.predict_proba(X_test)[:, 1]
  start = time.time()
  diabetesMedRF.fit(X_train, Y_diabetesMed_train)
  diabetesMedPredictions = diabetesMedRF.predict(X_test)
  diabetesMedProba = diabetesMedRF.predict_proba(X_test)[:, 1]
  diabetesMedRFAccuracy = accuracy_score(diabetesMedPredictions, Y_diabetesMed_test)
  ...
  for attr in sensitive_test: # race, age, gender
    ...
    # write my results to file (include "rf" in filenames)
    med_change_filename = f"base_rf_{attr}_medchange.csv"
    med_change_results.to_csv(med_change_filename, index=False)

    diabetes_med_given_filename = f"base_rf_{attr}_diabetesmed.csv"
    diabetes_med_given_results.to_csv(diabetes_med_given_filename, index=False)
    print("="*60)

  print("-"*50)
  print("BASE ACCURACIES (RANDOM FOREST)")
  print("-"*50)
  start = time.time()
  changeRF.fit(X_train, Y_change_train)
  ...
  print(f"Accuracy of change RANDOM FOREST: {changeRFAccuracy}")
  ...
  print(f"Accuracy of diabetesMed RANDOM FOREST: {diabetesMedRFAccuracy}")
  ...
  for attr in sensitive_train:
    ...
    for curr_strategy in strategies:
      ...
      med_change_filename = f"rf_{attr_strategy_change_str}.csv"
      med_change_results.to_csv(med_change_filename, index=False)

      diabetes_med_given_filename = f"rf_{attr_strategy_med_given_str}.csv"
      diabetes_med_given_results.to_csv(diabetes_med_given_filename, index=False)
