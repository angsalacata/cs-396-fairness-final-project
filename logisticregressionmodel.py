...
def reject_option_postprocess(
    ...
    # Define protected group: by default, everyone who is NOT in the majority group
...
def main():
  ...
  start = time.time()
  changeLogReg = LogisticRegression(
      C=1.0,
      max_iter=1000,
      solver='liblinear',
      random_state=42
  ...
  changeLogReg.fit(X_train, Y_change_train)
  changePredictions = changeLogReg.predict(X_test)
  changeProba = changeLogReg.predict_proba(X_test)[:, 1]
  changeLogRegAccuracy = accuracy_score(changePredictions, Y_change_test)
  changeTime = time.time() - start

  # target with diabetes meds
  diabetesMedLogReg = LogisticRegression(
      C=1.0,
      max_iter=1000,
      solver='liblinear',
      random_state=42
  ...
  start = time.time()
  diabetesMedLogReg.fit(X_train, Y_diabetesMed_train)
  diabetesMedPredictions = diabetesMedLogReg.predict(X_test)
  diabetesMedProba = diabetesMedLogReg.predict_proba(X_test)[:, 1]
  diabetesMedLogRegAccuracy = accuracy_score(diabetesMedPredictions, Y_diabetesMed_test)
  ...
  # points near the decision boundary get flipped in favor of protected groups.

  X_train_dense = X_train  # already dense after imputation
  X_test_dense = X_test

  fairness_optimized_predictions = {}
  fairness_optimized_accuracy = {}

  # Keep these as "labels" for your fair models, even though ROC is not explicitly
  # optimizing different fairness constraints here.
  ...
  for attr in sensitive_train:
    ...
    for curr_strategy in constraint_strategies:
      ...
      ######################### CHANGE LOGISTIC REGRESSION + ROC #########################
      fair_changePredictions = reject_option_postprocess(
          ...
          favorable_label=1,
          unfavorable_label=0,
          protected_values=None  # default: all non-majority groups are protected
      )
      ...
      ######################### DIABETES MED LOGISTIC REGRESSION + ROC ###################
  ...
  # 3 protected attrs * 2 targets * 2 strategy labels = 12
  ...
  for attr in sensitive_test: # race, age, gender
    ...
    # write my results to file (include "logistic" in filenames)
    med_change_filename = f"base_logistic_{attr}_medchange.csv"
    med_change_results.to_csv(med_change_filename, index=False)

    diabetes_med_given_filename = f"base_logistic_{attr}_diabetesmed.csv"
    diabetes_med_given_results.to_csv(diabetes_med_given_filename, index=False)
    print("="*60)

  print("-"*50)
  print("BASE ACCURACIES (LOGISTIC REGRESSION)")
  print("-"*50)
  print(f"CHANGE TIME: {changeTime}")
  print(f"Accuracy of change Logistic Regression: {changeLogRegAccuracy}")

  print(f"DIABETES MED TIME: {diabetesMedTime}")
  print(f"Accuracy of diabetesMed Logistic Regression: {diabetesMedLogRegAccuracy}")
  ...
  for attr in sensitive_train:
    ...
    for curr_strategy in strategies:
      ...
      # write my results to file, denoting logistic regression
      med_change_filename = f"logistic_{attr_strategy_change_str}.csv"
      med_change_results.to_csv(med_change_filename, index=False)

      diabetes_med_given_filename = f"logistic_{attr_strategy_med_given_str}.csv"
      diabetes_med_given_results.to_csv(diabetes_med_given_filename, index=False)
